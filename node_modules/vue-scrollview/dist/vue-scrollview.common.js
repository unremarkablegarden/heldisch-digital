 ...
 * @returns {Number}
 */
var getElDistanceTop = function (el) {
  var location = 0;
  if (el.offsetParent) {
    do {
 ...
 * @returns {Boolean}
 */
var keysAreUnique = function (state, children) {
  var exists = children.find(function (ref) {
    var key = ref.$vnode.key;

    var match = state.locations.find(function (ref) {
 ...
 */
var fetchComponentByKey = function (key, locations) { return locations.find(function (location) { return location.component === key; }); };
 ...
 * @returns null
 */
var createScrollListener = function (state) {
  var fns = [], len = arguments.length - 1;
  while ( len-- > 0 ) fns[ len ] = arguments[ len + 1 ];

  return function () {
    window.requestAnimationFrame(function () {
      fns.forEach(function (fn) { return fn(state); });
    });
  }
};
 ...
 * @returns null
 */
var checkInViewport = function (state) {
  state.locations.forEach(function (ref) {
    var position = ref.position;
    var scrollview = ref.scrollview;
    var component = ref.component;

    var ref$1 = state.scrollviews[scrollview];
    var offset = ref$1.offset;
    var withinTopBounds = (position.bottom - offset) - window.scrollY > 0;
    var withinBottomBounds = (position.top + offset) - window.scrollY <= window.innerHeight;
 ...
 * @returns null
 */
var broadcastScrollviews = function (ref) {
  var scrollviews = ref.scrollviews;
  var tracking = ref.tracking;

  Object.keys(scrollviews)
    .forEach(function (scrollview) { return scrollviews[scrollview].$emit('tracking:update', tracking[scrollview]); });
};
 ...
 * @returns null
 */
var resetScrollviews = function (state) {
  ...
  Object.entries(scrollviews)
    .forEach(function (ref) {
      var scrollview = ref[1];

      return scrollview.$emit('tracking:reset');
  });
 ...
 */
var initVueScrollview = function (ref) {
  if ( ref === void 0 ) ref = {};
 ...
 */
var initializeScrollview = function (ref) {
  var _uid = ref._uid;
  var $children = ref.$children;

  return $children.reduce(function (data, ref) {
    var $el = ref.$el;
    var child = ref.$vnode.key;
 ...
 */
var attachScrollListener = function (state) {
  state.scrollListener = index$1(
    createScrollListener.apply(void 0, [ state, checkInViewport, broadcastScrollviews ].concat( state.callbacks )),
    state.throttle,
    { leading: true }
  );
 ...
 * @returns {Object} ScrollView tracking state.
 */
var attachRecacheListener = function (state) {
  ...
  setInterval(function () { return checkIfRecache(state, function () { return resetScrollviews(state); }); }, 1000);
  return state
};
 ...
 * @returns null
 */
var checkIfRecache = function (state, resetFn) {
  var currentBottom = getElDistanceTop(state.recacheEl);
  if (currentBottom !== state.bottom) { resetFn(); }
  state.bottom = currentBottom;
};

var _private = function (state) { return ({
   ...
   * @returns null
   */
  _untrack: function (ref) {
    var _uid = ref._uid;

    delete state.tracking[_uid];
    state.locations = state.locations.filter(function (location) { return location.scrollview !== _uid; });
  }

}); };

var _public = function (state) { return ({
   ...
   * @returns null
   */
  scrollToComponent: function (key, offset) {
    var ref = fetchComponentByKey(key, state.locations);
    var position = ref.position;
    var scrollview = ref.scrollview;
    var ref$1 = state.scrollviews[scrollview];
    var defaultOffset = ref$1.offset;
  ...
  refresh: function () { return resetScrollviews(state); },
   ...
   * @returns {Number}
   */
  getComponentLocation: function (key) {
    var ref = fetchComponentByKey(key, state.locations);
    var position = ref.position;
    return position
  }
...
var ScrollView$$1 = {
  ...
  mounted: function mounted () {
    ...
    this.$on('tracking:update', function (update) { return this$1.tracking = update; });
    this.$on('tracking:reset', function () { return this$1.tracking = exports.$scrollview._track(this$1); });
  },
...
function plugin (Vue, options) {
  ...
  var state = initVueScrollview({ throttle: throttle, callbacks: callbacks });

  exports.$scrollview = Object.assign({}, {state: state},
    _private(state),
    _public(state));
